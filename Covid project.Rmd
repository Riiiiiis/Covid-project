...
all_merged_with_wuhan %>% filter(between(position,21500,22150)) %>% group_by(variant, Pairs) %>% summarise(n=n()) %>% filter(n < 10) %>%  spread(Pairs,n)
...
dk_wuhan_statcount_with_Variant %>% filter(count <= 5) %>% filter(between(position,21500,22150))
...
all_merged_with_wuhan %>% group_by(variant, Pairs) %>% filter(between(position, 21500,22150)) %>% summarise(n=n()) %>% filter(n < 5) %>% spread(key = "Pairs", value= n)
