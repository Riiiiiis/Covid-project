...
tidy_300K_fra <- mutate(tidy_300K, Date = format(substring(tidy_300K$`Collection date`,0,7)))

tidy_300K_fra <- tidy_300K_fra %>% mutate(mutations = (count_N + count_S))
...
tidy_300K_fra %>% 
  ...
  group_by(id, mutations, Date) %>%
  ggplot(tidy_300K_fra, mapping = aes(x=Date,y=mutations, color=pangolin_lineage)) +
  geom_point()
